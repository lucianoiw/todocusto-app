...
export function UnitsFilters({ workspaceSlug }: UnitsFiltersProps) {
  ...
  const updateFilters = useCallback(
    (updates: Record<string, string>) => {
      ...
      startTransition(() => {
        router.push(`/${workspaceSlug}/units?${params.toString()}`);
      });
    },
  ...
  const clearFilters = () => {
    setSearchValue("");
    startTransition(() => {
      router.push(`/${workspaceSlug}/units`);
    });
  };
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      >
        ...
        <option value="volume">Líquido</option>
