...
export function DeleteIngredientButton({
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const result = await deleteIngredient(workspaceSlug, ingredientId);
      if (result.error) {
        alert(result.error);
      } else {
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Tem certeza que deseja excluir "{ingredientName}"? Todas as variações e
            entradas também serão excluídas. Esta ação não pode ser desfeita.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
